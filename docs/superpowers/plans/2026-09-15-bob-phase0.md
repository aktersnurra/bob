...
**Tech Stack:** OCaml 5.2.0+ox, dune 3.23, `mtime` (monotonic timestamps), `yojson`, `uuidm`, `sqlite3` (FTS5), `alcotest` + `qcheck-alcotest`, `cmdliner`. Eio 1.5 is installed but **not used in Phase 0** — the core is pure and synchronous; Eio enters at Phase 1 when real IO loops appear.

**Verified before writing this plan** (compiled and run on this machine, not assumed):

- All libraries above build together; FTS5 queried from OCaml returns rows; a
  qcheck property test runs under alcotest.
...
- opam switch `5.2.0+ox` has: `eio_main.1.5`, `uuidm.0.9.10`, `qcheck-alcotest.0.91`, `sqlite3.5.4.2`, `yojson.2.2.2+ox`, `mtime.2.2.0`, `alcotest.1.9.0+ox`, `cmdliner.2.1.1`, `logs`, `fmt`, `ptime`, `re`.
