...
(package
 ...
 ;; allow_empty: until bin/ lands in Task 11 the package has no installable
 ;; stanza, and dune fails the default build without this.
 (allow_empty)
 (depends
  (ocaml (>= 5.2))
  dune
...
- [ ] **Step 2: Generate the opam file**

`generate_opam_files true` means dune writes `bob.opam` itself, and the project
will not build until that file is promoted into the tree:

Run: `opam exec -- dune build @install` then `opam exec -- dune promote`
Expected: `bob.opam` now exists in the project root.
...
(* `rec` matters: the Person_moved branch calls apply on a synthesised
   Person_entered when the track is not yet known. *)
...
Run: `opam exec -- dune test 2>&1 | tail -15`
Expected: PASS — `workspace` suite green, 10 tests.
...
(* Reserve room for the marker so the result never exceeds n. *)
let truncate n s =
...
#
# Ordering matters: the speaker stops talking (speech_ended) and only THEN does
# the final transcript resolve (utterance). A trace with utterance before
# speech_ended produces negative speech-end -> stt-final spans.
t=0.000 speech_started doa=-31 confidence=0.8
t=0.040 person_entered track=7 bearing=-29
t=0.220 person_identified track=7 person=gustaf confidence=0.93
t=0.400 partial_utterance text="Bob where..."
t=1.500 speech_ended
t=1.680 utterance speaker=gustaf text="Bob where did I put the screwdriver?"
```
...
- [ ] The replay shows Bob orienting at t=0.000s — before the utterance is
      final at t=1.680s. This is SPEC §34's central claim, demonstrated in
      simulation.
- [ ] No `INVALID` line appears in the latency report, and the replay emits no
      `interrupt_speech` decision on the first `speech_started`.
